...
--to upgrade/to churn dataset
, next_tx AS (
SELECT *
, LEAD(transaction_type1, 1) OVER (PARTITION BY cust_id ORDER BY transaction_date1) as subsequent_tx1
, LEAD(transaction_type1, 2) OVER (PARTITION BY cust_id ORDER BY transaction_date1) as subsequent_tx2
FROM pivot
ORDER BY cust_id, transaction_date1
)
...
, referral_type1
FROM next_tx
WHERE subsequent_tx1 = 'UPGRADE' --'CHURN'
OR subsequent_tx2 = 'UPGRADE' --'CHURN'
