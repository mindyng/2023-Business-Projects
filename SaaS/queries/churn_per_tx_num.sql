--churn is highest in beginning and then tapers off
...
SELECT TO_CHAR(SUM(CASE WHEN transaction_type1 = 'UPGRADE' THEN 1 ELSE 0 END) * 100.00/COUNT(*),'990D99%') AS first_tx_churn_pct
, TO_CHAR(SUM(CASE WHEN transaction_type2 = 'CHURN' THEN 1 ELSE 0 END) * 100.00/COUNT(*),'990D99%') AS second_tx_churn_pct
, TO_CHAR(SUM(CASE WHEN transaction_type3 = 'CHURN' THEN 1 ELSE 0 END) * 100.00/COUNT(*),'990D99%') AS third_tx_churn_pct
, TO_CHAR(SUM(CASE WHEN transaction_type4 = 'CHURN' THEN 1 ELSE 0 END) * 100.00/COUNT(*),'990D99%') AS fourth_tx_churn_pct
FROM pivot 
