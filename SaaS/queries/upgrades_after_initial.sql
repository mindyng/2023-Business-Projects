...
--upgrades after initial transaction
, perc AS (SELECT 
(SELECT COUNT(*)
FROM pivot
...
AND transaction_type2 = 'UPGRADE') *100.00
/
(SELECT COUNT(*) FROM pivot) AS upgrade_perc
)
...
--any upgrades after initial transaction
, perc AS (SELECT 
(SELECT COUNT(*)
FROM pivot
WHERE transaction_date3 IS NOT NULL
AND (transaction_type2 = 'UPGRADE'
	 OR transaction_type3 = 'UPGRADE'
)) 
*100.00
/
(SELECT COUNT(*) FROM pivot) AS upgrade_perc
